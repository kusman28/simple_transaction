...
<div class="container mx-auto p-4">
    ...
    @endif
    <form action="{{ route('transaction.update', $transaction->id) }}" method="POST" class="mt-4">
        @csrf
        @method('PUT')
        <select name="transaction_type" class="border p-2 rounded-lg w-full mb-4">
            ...
            @foreach(\App\Enums\TransactionTypes::types() as $type)
                <option value="{{ $type->value }}" {{ $transaction->transaction_type == $type->value ? 'selected' : '' }}>{{ ucfirst($type->value) }}</option>
            @endforeach
        </select>
        <div class="flex flex-col mb-4">
            <label for="amount" class="text-lg">Amount:</label>
            <input type="text" name="amount" id="amount" required class="border p-2 rounded-lg w-full" value="{{ old('amount', $transaction->amount) }}" />
        </div>
        <div class="flex flex-col mb-4">
            <label for="reference" class="text-lg">Reference:</label>
            <input type="text" name="reference" id="reference" required class="border p-2 rounded-lg w-full" value="{{ old('reference', $transaction->reference) }}" />
        </div>
